...
def make_graph(users: pd.DataFrame, stakes: pd.DataFrame) -> nx.DiGraph:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print('ERROR: python staking_directed_network.py dao_name')
        exit(1)
    ...
    stakes = stakes.sort_values(by=['createdAt'])

    graph: nx.DiGraph = make_graph(users=users, stakes=stakes)

    r: float = nx.degree_assortativity_coefficient(graph, weight='weight')
    print(f'\nAssortativity coefficient = {r:3.2f}\n')

    out_path: str = os.path.join('data', 'network', f'{sys.argv[1]}_stake_directed.gml')
