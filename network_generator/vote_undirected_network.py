...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3 or sys.argv[2] not in ['same', 'opposite']:
        print('ERROR: python vote_undirected_network.py dao_name [same, opposite]')
        exit(1)
    ...
    out_path: str = os.path.join('data', 'network', f'{sys.argv[1]}_vote_undirected.gml')
    if not same_vote:
        out_path = os.path.join('data', 'network', f'{sys.argv[1]}_opposite_vote_undirected.gml')
